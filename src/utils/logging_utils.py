...
def log_load_success(
    ...
    """
    Log successful data load with performance analysis.

    Args:
        logger: Logger instance to use for output.
        type: Description of the data type loaded.
        shape: Tuple of (rows, columns) loaded.
        execution_time: Time taken for load in seconds.
    """
    logger.info(f"Data load successful for {type}!")
    logger.info(f"Loaded {shape[0]} rows and {shape[1]} columns")
    logger.info(f"Execution time: {execution_time} seconds")

    if shape[0] > 0:
        logger.info(
            f"Execution time per row: {execution_time / shape[0]} seconds"
        )
    else:
