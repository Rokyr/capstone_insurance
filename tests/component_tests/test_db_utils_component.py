...
def test_get_db_connection_full_component_success(test_connection_params):
    ...
    ) as mock_create_engine, patch("src.utils.db_utils.logger") as mock_logger:
        # Act
...
def test_get_db_connection_engine_connects_but_connection_fails(
...
def test_get_db_connection_full_failure_chain(test_connection_params):
    ...
    with patch(
        ...
        side_effect=DatabaseConnectionError(
            "Failed to connect to the database: Database driver not found"
        ),
    ) as mock_create_engine, patch("src.utils.db_utils.logger") as mock_logger:
        # Act & Assert
        with pytest.raises(DatabaseConnectionError) as excinfo:
